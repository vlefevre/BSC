...
\acmConference[CGO’18]{International Symposium on Code Generation and Optimization}{February 24--28}{Vienna, Austria}
...
\begin{abstract}
  ...
  investigate whether it is possible or not to save time while keeping results that are not to far from their exact value. We first find a different cycle shape than the classic V-cycle used
  in multi-grid solvers that provides a usually faster convergence rate. Then we propose a dynamic bit-width changing algorithm that is able to adapt the execution time of floating-point operations
...
the number of evaluation points can vary) and the main idea of the multi-grid algorithms is to use different grids to determine a solution on an inaccurate grid that will be then used as the starting point of a solution on a more accurate grid. More precisely, a multi-grid algorithm has a parameter which is the cycle shape.
...
%     \includegraphics[width=\textwidth]{figs/3157_15.pdf}
   ...
   \begin{figure}
    \includegraphics[width=\linewidth]{figs/cost_3.pdf}
    \caption{}
