...
\subsection{An asymmetric strategy}
  ...
  \end{figure}
   ...
   to \texttt{float*} or \texttt{float}. For the second version, we use the library MPFR~\cite{MPFR} that introduces a type \texttt{mpfr\_t}. This type has a parameter which is the number of bits in the mantissa of the variable.
   ...
   Let us consider the equation $f(x_1,\dots,x_n) = \sum\limits_{i=1}^n x_i^2 - \sum\limits_{i=1}^{n-1} x_ix_{i+1}$. Finding the minimum of $f(x_1,\dots,x_{n-1},1)$ will give
