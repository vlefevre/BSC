...
\end{bmatrix} \textup{ and } b = \begin{bmatrix} 0 \\ \vdots \\ 0 \\ 1 \end{bmatrix} \]
   ...
   In order to estimate the cost of our algorithm, we want to model the time of an iteration at precision $b$. We model an iteration by the following formula: $\frac{a}{n^\beta}b^\alpha$, where $a,\beta$ and $\alpha$ are constants,
   $n$ is the number of processors and $b$ the number of bits.
