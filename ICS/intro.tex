...
\begin{itemize}
    ...
    \item We propose a new cycle configuration and show that it is more
        efficient than the state-of-the-art configurations by up to 28.3\%. We stress that we present only a few results because of lack of space but more measures were performed to confirm our deductions: we run the experiments on a total of 9 different matrices, with number of MPI processes ranging from 1 to 64, ech MPI process having only 1 OpenMP thread.
