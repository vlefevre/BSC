...
and execution time (or energy consumption) for MG algorithms. We focus our efforts on
...
we also propose a multi-grid algorithm to dynamically optimize the floating-point precision to increase the performance (or to reduce the energy consumption) while maintaining the quality of the solution.
...
\begin{itemize}
    ...
    \item We propose a new cycle configuration and show that it is more
        efficient than the state-of-the-art configurations by up to 28.3\%. We stress that we present only a few results because of lack of space but more measures were performed (and were consistent with the previous ones) to conclude on the \textsc{Up}-strategy: we run the experiments on a total of 9 different matrices, with number of MPI processes ranging from 1 to 64, ech MPI process having only 1 OpenMP thread.
