...
int
hypre_ParVectorScale( float        alpha,
                      hypre_ParVector *y     )
{
   hypre_Vector *y_local = hypre_ParVectorLocalVector(y);
...
int
hypre_ParVectorAxpy( float        alpha,
               hypre_ParVector *x,
               hypre_ParVector *y     )
{
   hypre_Vector *x_local = hypre_ParVectorLocalVector(x);
                    ...
                    hypre_ParVector *y )
{
   ...
   float result = 0.0;
   float local_result = hypre_SeqVectorInnerProd(x_local, y_local);
   
   MPI_Allreduce(&local_result, &result, 1, MPI_FLOAT, MPI_SUM, comm);
...
/* initialize data exchange among used_procs and generate vector  - here we 
   ...
   for (i = 0; i < num_types; i++)
   {
        ...
        MPI_Irecv(&vector_data[(int) new_vec_starts[i]], num_vectors*vec_len, MPI_FLOAT,
                                proc_id, tag2, comm, &requests[j++]);
   }
   for (i = 0; i < num_types; i++)
   {
        MPI_Isend(local_data, num_vectors*local_size, MPI_FLOAT, used_procs[i],
...
/* initialize data exchange among used_procs and generate vector */
   ...
   for (i = 0; i < num_types; i++)
   {
        ...
        MPI_Irecv(&vector_data[vec_starts[proc_id]], num_vectors*vec_len, MPI_FLOAT,
