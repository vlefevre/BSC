...
int
hypre_ParCSRMatrixMatvec( float           alpha,
                 ...
                 hypre_ParVector    *y     )
{
   ...
   float     *x_tmp_data, **x_buf_data;
   float     *x_local_data = hypre_VectorData(x_local);
   /*---------------------------------------------------------------------
   ...
   x_buf_data = hypre_CTAlloc( float*, num_vectors );
   for ( jv=0; jv<num_vectors; ++jv )
      x_buf_data[jv] = hypre_CTAlloc(float, hypre_ParCSRCommPkgSendMapStart
                                    (comm_pkg, num_sends));
...
int
hypre_ParCSRMatrixMatvecT( float           alpha,
                  hypre_ParCSRMatrix *A,
                  hypre_ParVector    *x,
                  float           beta,
                  hypre_ParVector    *y     )
{
   ...
   float       *y_tmp_data, **y_buf_data;
   float       *y_local_data = hypre_VectorData(y_local);
   ...
   num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
   y_buf_data = hypre_CTAlloc( float*, num_vectors );
   for ( jv=0; jv<num_vectors; ++jv )
      y_buf_data[jv] = hypre_CTAlloc(float, hypre_ParCSRCommPkgSendMapStart
                                     (comm_pkg, num_sends));
...
int
hypre_ParCSRMatrixMatvec_FF( float           alpha,
                 hypre_ParCSRMatrix *A,
                 hypre_ParVector    *x,
                 float           beta,
                 hypre_ParVector    *y,
                 ...
                 int fpt )
{
   ...
   float     *x_tmp_data = NULL;
   float     *x_buf_data = NULL;
   float     *x_local_data = hypre_VectorData(x_local);
   /*---------------------------------------------------------------------
    *  Check for size compatibility.  ParMatvec returns ierr = 11 if
    *  length of X doesn't equal the number of columns of A,
    ...
    *--------------------------------------------------------------------*/
      ...
      num_sends = hypre_ParCSRCommPkgNumSends(comm_pkg);
      if (num_sends)
         x_buf_data = hypre_CTAlloc(float, hypre_ParCSRCommPkgSendMapStart
                                    (comm_pkg, num_sends));
