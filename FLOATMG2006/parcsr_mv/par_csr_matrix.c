...
hypre_ParCSRMatrixSetDNumNonzeros( hypre_ParCSRMatrix *matrix)
{
   ...
   int *offd_i;
   int local_num_rows;
   float total_num_nonzeros;
   float local_num_nonzeros;
   if (!matrix)
   {
      hypre_error_in_arg(1);
      return hypre_error_flag;
   ...
   offd_i = hypre_CSRMatrixI(offd);
   local_num_rows = hypre_CSRMatrixNumRows(diag);
   local_num_nonzeros = (float) diag_i[local_num_rows] 
		+ (float) offd_i[local_num_rows];
   MPI_Allreduce(&local_num_nonzeros, &total_num_nonzeros, 1, MPI_FLOAT,
        MPI_SUM, comm);
...
{  
    ...
    */
     ...
     hypre_ParCSRMatrixRowvalues(mat) = (float *) hypre_CTAlloc
                             ( float, max ); 
                    ...
                    HYPRE_BigInt last_col_diag)
{
   ...
   float *a_data = hypre_CSRMatrixData(A);
...
/* generate datatypes for further data exchange and exchange remaining
   ...
   for (i = 0; i < num_types; i++)
   {
        ...
        MPI_Irecv(&matrix_data[start_index], num_data, MPI_FLOAT,
   ...
   for (i=0; i < num_types; i++)
   {
        MPI_Isend(local_matrix_data, local_num_nonzeros, MPI_FLOAT,
                        used_procs[i], tag1, comm, &requests[j++]);
