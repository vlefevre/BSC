...
*/
{
   ...
   */
   ...
   float          *A_diag_data = hypre_CSRMatrixData(A_diag);
   int             *A_diag_i = hypre_CSRMatrixI(A_diag);
   int             *A_diag_j = hypre_CSRMatrixJ(A_diag);

   hypre_CSRMatrix *A_offd = hypre_ParCSRMatrixOffd(A);
   
   float          *A_offd_data = hypre_CSRMatrixData(A_offd);
   ...
   float          *P_diag_data = hypre_CSRMatrixData(P_diag);
   ...
   float          *P_offd_data = hypre_CSRMatrixData(P_offd);
   ...
   float          *P_ext_diag_data;
   int             *P_ext_diag_i;
   int             *P_ext_diag_j;
   int              P_ext_diag_size;

   float          *P_ext_offd_data;
   int             *P_ext_offd_i;
   int             *P_ext_offd_j;
   HYPRE_BigInt    *P_ext_tmp_j;
   int              P_ext_offd_size;
...
/* no changes for the marked version above this point */
   ...
   */
   ...
   C_diag_data = hypre_CTAlloc(float, C_diag_size);
   C_diag_j    = hypre_CTAlloc(int, C_diag_size);
   if (C_offd_size)
   { 
   	C_offd_data = hypre_CTAlloc(float, C_offd_size);
...
void hypre_ParMatScaleDiagInv_F(
   hypre_ParCSRMatrix * C, hypre_ParCSRMatrix * A, float weight, int * CF_marker )
    ...
    */
{
   ...
   */
   ...
   int             *A_diag_i = hypre_CSRMatrixI(A_diag);
   int             *A_diag_j = hypre_CSRMatrixJ(A_diag);
   float          *C_diag_data = hypre_CSRMatrixData(C_diag);
   float          *C_offd_data = hypre_CSRMatrixData(C_offd);
...
*/
{
   ...
   */
   ...
   int             *Pnew_diag_i;
   int             *Pnew_diag_j;
   float          *Pnew_diag_data;
   int             *Pnew_offd_i;
   int             *Pnew_offd_j;
   float          *Pnew_offd_data;
   int             *Pnew_j2m;
