  ...
  /* ----------------------------GATHER? ------------------------------------*/  
   ...
   if (global_is_gather)
   {
...
#ifdef HYPRE_NO_GLOBAL_PARTITION
      {
         ...
         float gamma;
         float local_volume, global_volume;
         float sendbuf2[2], recvbuf2[2];
         ...
         /* create an array of local boxes.  also get the bounding box 
            and the global box size (as a float).  we don't use the bounding box
         ...
         for (i=0; i< num_my_entries; i++)
         {
            ...
            local_volume += (float) hypre_BoxVolume(box);
         ...
         sendbuf2[1] = (float) num_my_entries;
         
         MPI_Allreduce(&sendbuf2, &recvbuf2, 2, MPI_FLOAT, MPI_SUM, comm);   
