...
int hypre_APGetAllBoxesInRegions( hypre_BoxArray*, hypre_BoxArray *, int**, 
                                  float**, MPI_Comm);
int hypre_APPruneRegions( hypre_BoxArray*,  int** , float**);
int hypre_APShrinkRegions( hypre_BoxArray*, hypre_BoxArray*, MPI_Comm );
int hypre_APRefineRegionsByVol( hypre_BoxArray*,  float*, int, float, 
                                int, int *, MPI_Comm);
...
int hypre_APFindMyBoxesInRegions( hypre_BoxArray *region_array, 
                                  ...
                                  int **p_count_array, float **p_vol_array)
{
   ...
   for (i=0; i< num_regions; i++)
   {
	 ...
	 else
	 {
	   ...
	   hypre_IntersectBoxes( my_box, region, result_box);
         ...
         {
            count_array[i]++;            
            vol_array[i] += (float) hypre_BoxVolume(result_box);
         }
      }
   }
                                  ...
                                  MPI_Comm comm )
{
   ...
   send_buf_vol =  hypre_CTAlloc(float, num_regions*2); /*allocate float*/

   dbl_vol_and_count =  hypre_CTAlloc(float, num_regions*2); /*allocate float*/

   hypre_APFindMyBoxesInRegions( region_array, my_box_array, &send_buf_count, 
                                 &send_buf_vol);


   /* copy ints to floats so we can do one all_reduce */
   for (i=0; i< num_regions; i++)
   {
      send_buf_vol[num_regions+i] = (float) send_buf_count[i];
   }

   MPI_Allreduce(send_buf_vol, dbl_vol_and_count, num_regions*2, MPI_FLOAT, 
                 MPI_SUM, comm);
...
int hypre_APRefineRegionsByVol( hypre_BoxArray *region_array,  float *vol_array, 
                        int max_regions, float gamma, int dim, int *return_code, 
                                MPI_Comm comm)
{
   ...
   fraction_full = hypre_CTAlloc(float,  num_regions); 
   ...
   for (i= 0; i< num_regions; i++)
   {
      ...
      fraction_full[i] = (vol_array[i])/((float) hypre_floatBoxVolume(box));
                                       ...
                                       hypre_StructAssumedPart **p_assumed_partition)
{
   ...
   float             *vol_array=NULL, one_volume;
   int                return_code;
   int                num_refine;
   ...
   float             f1, f2, r,x_box, y_box, z_box, dbl_vol;
   int                initial_level;
   int                grow, grow_array[6];
   ...
   /* note: this doesn't change the value in the caller's code */
    ...
    /* one_volume = (float) hypre_BoxSizeX(bounding_box) * 
       (float) hypre_BoxSizeY(bounding_box) * (float) hypre_BoxSizeZ(bounding_box); */
    
    one_volume = (float) hypre_floatBoxVolume(bounding_box);
   ...
   vol_array =  hypre_CTAlloc(float,  size); 
   ...
   if ( !(size ==1 && max_refinements == 0))
   {
           ...
           (num_refine < max_refinements))
    {
       ...
       if (size >  count_array_size)
       {
          ...
          vol_array =  hypre_TReAlloc(vol_array, float,  size); 
...
/* ------------------------------------------------------------------------*/
   ...
   /*the above is aliased - so don't destroy region_array in this function */
    ...
    for (i=0; i < size; i++) 
    {
       ...
       {
          proc_array[i] = (int) hypre_round(((float)count_array[i]/ 
                                             (float)total_boxes)* (float) num_procs);
       }
       
       box =  hypre_BoxArrayBox(region_array, i); 
       /* vol = hypre_BoxVolume(box); */
       dbl_vol = hypre_floatBoxVolume(box);
       
       /* can't have any zeros! */
       if (!proc_array[i]) proc_array[i] = 1;


       if (dbl_vol < (float) proc_array[i]) 
       {
    ...
    while (num_procs > proc_count)
    {
       ...
       if ( (float) proc_array[max_position] >  
           hypre_floatBoxVolume(hypre_BoxArrayBox(region_array, max_position))) 
       {
          proc_array[max_position]--;
    ...
    /* now determine the NUMBER of divisions in the x, y amd z dir according
     ...
     for (i = 0; i< size; i++)
     {
        proc_count =  proc_array[i];
        box =  hypre_BoxArrayBox(region_array, i);
        x_box =  (float) hypre_BoxSizeX(box);
        y_box =  (float) hypre_BoxSizeY(box);
        z_box =  (float) hypre_BoxSizeZ(box);
        ...
        if (dim == 2)
        {
           ...
           /*case 1*/
           r = (float) proc_count/ (float) i1;
           ti1 = i1;
           t_tmp = (int) ceil(sqrt(r));
           t_total =  t_tmp*t_tmp*i1;
           
           /* is this better?*/ 
           r = (float) proc_count/ (float) i11;
           tmp = (int) ceil(sqrt(r));          
           tmp_num =  tmp*tmp*i11;
        ...
        else
        {
           ...
           r = (float) proc_count / (float) (i2*i1);
           ...
           /* can we do better ? */  
           /*case 1: i11 and i2 */           
           r = (float) proc_count / (float) (i2*i11);
           tmp = (int) ceil(pow(r,(1.0/3.0)));
           ...
           /*case 2: i11 and i22 */           
           r = (float) proc_count / (float) (i22*i11);
           tmp = (int) ceil(pow(r,(1.0/3.0)));
           ...
           /*case 3: i1 and i22 */           
           r = (float) proc_count / (float) (i22*i1);
           tmp = (int) ceil(pow(r,(1.0/3.0)));
