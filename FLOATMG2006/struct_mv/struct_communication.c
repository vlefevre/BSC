...
/* this computes a (large enough) size (in floats) for the message prefix */
#define hypre_CommPrefixSize(ne) \
( ((ne*sizeof(int) + ne*sizeof(hypre_Box))/sizeof(float)) + 1 )
                               ...
                               hypre_CommHandle **comm_handle_ptr )
{
   ...
   float              *dptr, *iptr, *jptr, *kptr, *lptr;
   int                 *qptr;
   ...
   send_buffers = hypre_TAlloc(float *, num_sends);
   if (num_sends > 0)
   {
      size = hypre_CommPkgSendBufsize(comm_pkg);
      send_buffers[0] = hypre_SharedTAlloc(float, size);
   ...
   recv_buffers = hypre_TAlloc(float *, num_recvs);
   if (num_recvs > 0)
   {
      size = hypre_CommPkgRecvBufsize(comm_pkg);
      recv_buffers[0] = hypre_SharedTAlloc(float, size);
         ...
         for (ll = 0; ll < length_array[3]; ll++)
         {
            ...
            for (kk = 0; kk < length_array[2]; kk++)
            {
               ...
               for (jj = 0; jj < length_array[1]; jj++)
               {
                  if (stride_array[0] == 1)
                  {
                     memcpy(dptr, jptr, length_array[0]*sizeof(float));
                  }
                  else
                  {
                     iptr = jptr;
   ...
   for(i = 0; i < num_recvs; i++)
   {
      ...
      MPI_Irecv(recv_buffers[i],
                hypre_CommTypeBufsize(comm_type)*sizeof(float), MPI_BYTE,
...
hypre_FinalizeCommunication( hypre_CommHandle *comm_handle )
{
   ...
   float         **send_buffers = hypre_CommHandleSendBuffers(comm_handle);
   float         **recv_buffers = hypre_CommHandleRecvBuffers(comm_handle);
   int              num_sends    = hypre_CommPkgNumSends(comm_pkg);
   int              num_recvs    = hypre_CommPkgNumRecvs(comm_pkg);
         ...
         for (ll = 0; ll < length_array[3]; ll++)
         {
            ...
            for (kk = 0; kk < length_array[2]; kk++)
            {
               ...
               for (jj = 0; jj < length_array[1]; jj++)
               {
                  if (stride_array[0] == 1)
                  {
                     memcpy(jptr, dptr, length_array[0]*sizeof(float));
                  }
                  else
                  {
                     iptr = jptr;
