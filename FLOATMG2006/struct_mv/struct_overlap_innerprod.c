...
float          *local_result_ref[hypre_MAX_THREADS];
#endif

float           final_innerprod_result;
...
{
   float               local_result, overlap_result;
   float               process_result;
   ...
   MPI_Allreduce(&process_result, &final_innerprod_result, 1,
                 MPI_FLOAT, MPI_SUM, hypre_StructVectorComm(x));
