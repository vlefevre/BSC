...
hypre_StructMatrixInitialize( hypre_StructMatrix *matrix )
{
   ...
   data = hypre_SharedCTAlloc(float, hypre_StructMatrixDataSize(matrix));
...
hypre_StructMatrixAssemble( hypre_StructMatrix *matrix )
{
   ...
   float                *matrix_data = hypre_StructMatrixData(matrix);
   float                *matrix_data_comm = matrix_data;
   ...
   if (!comm_pkg)
   {
      ...
      if (sum_OffProcAdd)
      {
         ...
         /* inverse CommInfo achieved by switching send & recv structures of
          ...
          data= hypre_CTAlloc(float, hypre_StructMatrixDataSize(matrix));
                           ...
                           hypre_StructMatrix *to_matrix   )
{
   ...
   hypre_StructStencil   *stencil;
   int                    data_initial_offset = 0;
   float                *matrix_data_from = hypre_StructMatrixData(from_matrix);
   float                *matrix_data_to = hypre_StructMatrixData(to_matrix);
   float                *matrix_data_comm_from = matrix_data_from;
   float                *matrix_data_comm_to = matrix_data_to;
