                   ...
                   hypre_ParCSRMatrix **P_ptr )
{
   ...
   float          *Pext_send_data = NULL;
   float          *Pext_data = NULL;

   float           sum_C, sum_N;
   float           sum_C_pos, sum_C_neg;
   float           sum_N_pos, sum_N_neg;
   float           diagonal;
   float           alfa = 1.0;
   float           beta = 1.0;
   int              j_start;
   ...
   P_diag_data = hypre_CTAlloc(float,total_nz);

   if (total_nz_offd)
   {
      P_offd_j = hypre_CTAlloc(int,total_nz_offd);
      P_offd_data = hypre_CTAlloc(float,total_nz_offd);
   }
      ...
      for (pass = 2; pass < num_passes; pass++)
      {
         ...
         if (num_procs > 1)
         {
            ...
            if (Pext_send_size > old_Pext_send_size)
            {
               ...
               Pext_send_data = hypre_CTAlloc(float, Pext_send_size);
            }
            old_Pext_send_size = Pext_send_size;
		...
		Pext_recv_vec_start[pass];
            ...
            if (Pext_recv_size > old_Pext_recv_size)
            {
               ...
               Pext_data = hypre_CTAlloc(float, Pext_recv_size);
            }
            old_Pext_recv_size = Pext_recv_size;
