                         ...
                         hypre_ParCSRMatrix  **P_ptr)
{
   ...
   float          *P_diag_data = NULL;
   int             *P_diag_i;
   int             *P_diag_j = NULL;
   float          *P_offd_data = NULL;
   int             *P_offd_i;
   int             *P_offd_j = NULL;
   ...
   float           wall_time;  /* for debugging instrumentation  */
...
#include "../utilities/hypre_smp_forloop.h"
   ...
   if (P_diag_size)
   {
      ...
      P_diag_data = hypre_CTAlloc(float, P_diag_size);
   }
   ...
   if (P_offd_size)
   {
      ...
      P_offd_data = hypre_CTAlloc(float, P_offd_size);
   }
				 ...
				 int max_elmts)        
{
   ...
   if (max_elmts > 0)
   {
    ...
    if (P_mxnum > max_elmts)
    {
     ...
     P_aux_data = hypre_CTAlloc(float, P_mxnum);
     cnt_diag = 0;
     cnt_offd = 0;
   ...
   if (num_lost)
   {
      ...
      P_diag_data_new = hypre_CTAlloc(float,P_diag_size);
   ...
   if (num_lost_offd)
   {
      ...
      P_offd_data_new = hypre_CTAlloc(float,P_offd_size);
      for (i=0; i < P_offd_size; i++)
      {
