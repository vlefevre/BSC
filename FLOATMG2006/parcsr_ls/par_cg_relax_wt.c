			  ...
			  float 	    *rlx_wt_ptr)
{
   ...
   float   *tridiag;
   float   *trioffd;
   float    alphinv, row_sum = 0;
   float    max_row_sum = 0;
   float    rlx_wt = 0;
   float    rlx_wt_old = 0;
   float    lambda_max, lambda_max_old;
   /* float    lambda_min, lambda_min_old; */
   ...
   tridiag  = hypre_CTAlloc(float, num_cg_sweeps+1);
   trioffd  = hypre_CTAlloc(float, num_cg_sweeps+1);
...
int
hypre_Bisection(int n, float *diag, float *offd, 
		float y, float z,
		float tol, int k, float *ev_ptr)
{
   float x;
   float eigen_value;
   int ierr = 0;
