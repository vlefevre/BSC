		 ...
		 HYPRE_ParVector *x_ptr)
{
   ...
   float hhx, hhy, hhz;
   float xx, yy, zz;
   float afp, afm, bfp, bfm, cfp, cfm, df, ef, ff, gf;
   ...
   rhs_data = hypre_CTAlloc(float, local_num_rows);
   x_data = hypre_CTAlloc(float, local_num_rows);
   ...
   diag_data = hypre_CTAlloc(float, diag_i[local_num_rows]);

   if (num_procs > 1)
   {
      offd_j = hypre_CTAlloc(int, offd_i[local_num_rows]);
      offd_data = hypre_CTAlloc(float, offd_i[local_num_rows]);
   }
   ...
   for (iz = 0; iz < nz_local; iz++)
   {
      zz = (float)(nz_part[ir]+iz+1)*hhz;
      for (iy = 0;  iy < ny_local; iy++)
      {
         yy = (float)(ny_part[iq]+iy+1)*hhy;
         for (ix = 0; ix < nx_local; ix++)
         {
            xx = (float)(nx_part[ip]+ix+1)*hhx;
