...
hypre_BoomerAMGBuildStdInterp(hypre_ParCSRMatrix *A, int *CF_marker,
			      ...
			      float trunc_factor, int max_elmts, 
			      ...
			      hypre_ParCSRMatrix  **P_ptr)
{
  ...
  float          *ahat, *ahat_offd = NULL; 
  float           sum_pos, sum_pos_C, sum_neg, sum_neg_C, sum, sum_C;
  float           diagonal, distribute;
  float           alfa, beta;
  ...
  MPI_Comm_rank(comm,&my_id);
   ...
   if (n_fine)
   {
      ahat = hypre_CTAlloc(float, n_fine);
      ihat = hypre_CTAlloc(int, n_fine);
      ipnt = hypre_CTAlloc(int, n_fine);
   }
   if (full_off_procNodes)
   {
     ahat_offd = hypre_CTAlloc(float, full_off_procNodes);
