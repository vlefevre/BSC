...
int
hypre_ParCSRMatrixScaledNorm( hypre_ParCSRMatrix *A, float *scnorm)
{
   ...
   d_buf_data = hypre_CTAlloc(float, hypre_ParCSRCommPkgSendMapStart(comm_pkg,
						num_sends));
   ...
   MPI_Allreduce(&max_row_sum, &mat_norm, 1, MPI_FLOAT, MPI_MAX, comm);
