                       ...
                       hypre_ParCSRMatrix   **AN_ptr)
{
  ...
  for (i=0; i < num_procs+1; i++)
   ...
   AN_diag_data = hypre_CTAlloc(float, AN_num_nonzeros_diag);	
   ...
   if (AN_num_nonzeros_offd)
   {
      ...
      AN_offd_data = hypre_CTAlloc(float, AN_num_nonzeros_offd);	
                       ...
                       hypre_ParCSRMatrix   **S_ptr)
{
   ...
   *CF_marker_ptr = CF_marker;
   ...
   if (data) 
   {
      ...
      S_diag_data = hypre_CTAlloc(float, S_num_nonzeros_diag);
      hypre_CSRMatrixData(S_diag) = S_diag_data;
      if (num_cols_offd_S)
      {
         SN_offd_data = hypre_CSRMatrixData(SN_offd);
         S_offd_data = hypre_CTAlloc(float, S_num_nonzeros_offd);
