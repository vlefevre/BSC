                        ...
                        hypre_ParCSRMatrix *A         )
{
   ...
   int       global_min_e;
   int       global_max_e;
   float    global_min_rsum;
   float    global_max_rsum;
   float    global_min_wt;
   float    global_max_wt;
   ...
   send_buff     = hypre_CTAlloc(float, 6);
#ifdef HYPRE_NO_GLOBAL_PARTITION
   gather_buff = hypre_CTAlloc(float,6);    
#else
   gather_buff = hypre_CTAlloc(float,6*num_procs);    
#endif
   ...
   { 

      {
         ...
         num_variables[level] = (float) fine_size;
         
         sparse = global_nonzeros /((float) fine_size * (float) fine_size);
         ...
         avg_entries = global_nonzeros / ((float) fine_size);
      }
       ...
       send_buff[0] = - (float) min_entries;
       send_buff[1] = (float) max_entries;
       send_buff[2] = - min_rowsum;
       send_buff[3] = max_rowsum;

       MPI_Reduce(send_buff, gather_buff, 4, MPI_FLOAT, MPI_MAX, 0, comm);
       ...
       MPI_Gather(send_buff,4,MPI_FLOAT,gather_buff,4,MPI_FLOAT,0,comm);
   ...
   for (level = 0; level < num_levels-1; level++)
   {
    
      {
         ...
         avg_entries = ((float) global_nonzeros) / ((float) fine_size);
      }
      ...
      MPI_Reduce(send_buff, gather_buff, 6, MPI_FLOAT, MPI_MAX, 0, comm);
      ...
      MPI_Gather(send_buff,6,MPI_FLOAT,gather_buff,6,MPI_FLOAT,0,comm);
