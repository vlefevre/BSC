...
void hypre_BoomerAMGJacobiInterp( hypre_ParCSRMatrix * A,
                                  ...
                                  float truncation_threshold_minus )
/* nji steps of Jacobi interpolation, with nji presently just set in the code.*/
{
   float weight_AF = 1.0;  /* weight multiplied by A's fine row elements */
 ...
 */
{
   ...
   int m, nmav, npav;
   float PIi, PIimax, PIimin, PIimav, PIipav, randthresh;
   float eps = 1.0e-17;
#endif
   ...
   randthresh = 15/(float)Jchanges;
   for ( i=0; i<num_rows_diag_P; ++i )
   {
      if ( J_marker[i]<0 )
      {
         if ( ((float)rand())/RAND_MAX < randthresh )
         {
            printf( "%i: ", i );
            for ( m=P_diag_i[i]; m<P_diag_i[i+1]; ++m )
...
*/
{
   ...
   float vmax = 0.0;
   float vmin = 0.0;
   float v, old_sum, new_sum, scale, wmax, wmin;
   int i1, m, m1d, m1o;
   ...
   */
   ...
   MPI_Allreduce( &vmax1, &vmax, 1, MPI_FLOAT, MPI_MAX, comm );
   MPI_Allreduce( &vmin1, &vmin, 1, MPI_FLOAT, MPI_MIN, comm );
#endif
