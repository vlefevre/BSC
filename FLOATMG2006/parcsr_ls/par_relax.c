                        ...
                        hypre_ParVector    *Vtemp )
{
   ...
   float         *u_data  = hypre_VectorData(u_local);

   hypre_Vector   *f_local = hypre_ParVectorLocalVector(f);
   float         *f_data  = hypre_VectorData(f_local);

   hypre_Vector   *Vtemp_local = hypre_ParVectorLocalVector(Vtemp);
   float         *Vtemp_data = hypre_VectorData(Vtemp_local);
   float 	  *Vext_data;
   float 	  *v_buf_data;
   float 	  *tmp_data;
...
#include "../utilities/hypre_smp_forloop.h"
	...
	if (p != my_id)
	{
	    ...
	    if (ip == p)
	    {
	       ...
	       MPI_Isend(&v_buf_data[vec_start], vec_len, MPI_FLOAT,
	  ...
	  for (i = 0; i < num_recvs; i++)
          {
             ...
             MPI_Irecv(&Vext_data[vec_start], vec_len, MPI_FLOAT,
                        ip, 0, comm, &requests[jr++]);
	  }
	  MPI_Waitall(jr,requests,status);
	  }
