...
char infile_default[50] = "sstruct.in.AMG.FD";
...
int BuildParLaplacian (int argc , char *argv [], float *system_size_ptr, HYPRE_ParCSRMatrix *A_ptr , HYPRE_ParVector *rhs_ptr , HYPRE_ParVector *x_ptr);
/* Laplace type problem, with 27pt stencil on a cube */
int BuildParLaplacian27pt (int argc , char *argv [], float *system_size_ptr, HYPRE_ParCSRMatrix *A_ptr , HYPRE_ParVector *rhs_ptr , HYPRE_ParVector *x_ptr);
/* PDE with jumps on a cube */
int BuildParVarDifConv (int argc , char *argv [], float *system_size_ptr, HYPRE_ParCSRMatrix *A_ptr , HYPRE_ParVector *rhs_ptr , HYPRE_ParVector *x_ptr);
   ...
   while (sdata_line < (sdata + sdata_size))
   {
      ...
      if ( ( sscanf(sdata_ptr, "%s", key) > 0 ) && ( sdata_ptr[0] != '#' ) )
      {
         ...
         else if ( strcmp(key, "StencilCreate:") == 0 )
         {
            ...
            data.stencil_values  = hypre_CTAlloc(float *, data.nstencils);
            SScanIntArray(sdata_ptr, &sdata_ptr,
            ...
            for (s = 0; s < data.nstencils; s++)
            {
               ...
               data.stencil_values[s] =
                  hypre_CTAlloc(float, data.stencil_sizes[s]);
            }
         }
         ...
         else if ( strcmp(key, "GraphAddEntries:") == 0 )
         {
            ...
            if ((pdata.graph_nboxes % 10) == 0)
            {
               ...
               pdata.graph_values =
                  hypre_TReAlloc(pdata.graph_values, float, size);
         ...
         else if ( strcmp(key, "MatrixSetValues:") == 0 )
         {
            ...
            if ((pdata.matset_nboxes % 10) == 0)
            {
               ...
               pdata.matset_values =
                  hypre_TReAlloc(pdata.matset_values, float, size);
            }
            SScanProblemIndex(sdata_ptr, &sdata_ptr, data.ndim,
         ...
         else if ( strcmp(key, "MatrixAddToValues:") == 0 )
         {
            ...
            if ((pdata.matadd_nboxes% 10) == 0)
            {
               ...
               pdata.matadd_values=
                  hypre_TReAlloc(pdata.matadd_values, float *, size);
            }
            SScanProblemIndex(sdata_ptr, &sdata_ptr, data.ndim,
            ...
            pdata.matadd_values[pdata.matadd_nboxes] =
               hypre_TAlloc(float, i);
            SScanDblArray(sdata_ptr, &sdata_ptr, i,
              (float *) pdata.matadd_values[pdata.matadd_nboxes]);
            pdata.matadd_nboxes++;
            data.pdata[part] = pdata;
         }
                ...
                ProblemData  *data_ptr )
{
   ...
   {
      printf("Error: Invalid number (%d VS %d) of processes or process topology \n",pool_procs[data.npools],num_procs);
      exit(1);
   }
            ...
            int   myid )
{
   if ( myid == 0 )
   {
      ...
      printf("                        0 - PCG with AMG (default) precond\n");
      printf("                        1 - PCG with AMG (low complexity) precond\n");
      printf("                        2 - PCG with diagonal scaling\n");
      printf("                        3 - GMRES(10) with AMG (default) precond\n");
      printf("                        4 - GMRES(10) with AMG (low complexity) precond\n");
      printf("                        5 - GMRES(10) with diagonal scaling\n\n");
      ...
      char *argv[] )
{
   ...
   int			 val_max_iter = 1000;
   float		 val_tolerance = 0;
   int			 val_cycle_type = 1;
   int			 val_num_relax = 1;
   int                   print_system = 0;
   int                   print_level = 0;
   ...
   while ( (arg_index < argc))
   {
	 ...
	 if (solver_id < 0 || solver_id > 5)
	 {
      ...
      {
         arg_index++;
	 val_tolerance = atof(argv[arg_index++]);
      }
      else if ( strcmp(argv[arg_index], "-maxiter") == 0 )
      {
         arg_index++;
	 val_max_iter = atoi(argv[arg_index++]);
      }
      else if ( strcmp(argv[arg_index], "-cycle") == 0 )
      {
         arg_index++;
	 val_cycle_type = atoi(argv[arg_index++]);
      }
      else if ( strcmp(argv[arg_index], "-relax") == 0 )
      {
         arg_index++;
	 val_num_relax = atoi(argv[arg_index++]);
      }
      else
         arg_index++;
   }
   ...
   while (arg_index < argc)
   {
      ...
      else*/ if ( strcmp(argv[arg_index], "-r") == 0 )
      {
         ...
         system_size *= (float)r_xyz[0]*(float)r_xyz[1]*(float)r_xyz[2];
         hypre_TFree(r_xyz);
      }
      else if ( strcmp(argv[arg_index], "-P") == 0 )
      {
         ...
         system_size *= (float)P_xyz[0]*(float)P_xyz[1]*(float)P_xyz[2];
         hypre_TFree(P_xyz);
      }
   ...
   values = hypre_TAlloc(float, data.max_boxsize);
...
/* since we have already tested SetBoxValues above, use SetValues here */
   ...
   if ((solver_id > -1) && (solver_id < 3))
   {
      ...
      if (solver_id == 0)
      {
         ...
         HYPRE_BoomerAMGSetMaxIter(par_precond, val_max_iter);
	 HYPRE_BoomerAMGSetSmoothNumSweeps(par_precond,val_num_relax);
	 HYPRE_BoomerAMGSetCycleType(par_precond,val_cycle_type);
         HYPRE_PCGSetPrecond( par_solver,
                             (HYPRE_PtrToSolverFcn) HYPRE_BoomerAMGSolve,
                             (HYPRE_PtrToSolverFcn) HYPRE_BoomerAMGSetup,
                             par_precond );

	HYPRE_BoomerAMGSetup(par_precond,(void *) par_A, (void *) par_b, (void *) par_x);
      time_index = hypre_InitializeTiming("Multi-grid");
      hypre_BeginTiming(time_index);
	int valentin_cpt = 0;
	for (valentin_cpt=0; valentin_cpt < 1; valentin_cpt++)
	{
		 HYPRE_ParCSRMatrixMatvec(0.0, par_A, par_b, 0.0, par_x );
	      HYPRE_BoomerAMGSolve( par_precond, (void*) par_A,
			      (void*) par_b, (void*) par_x );
	}
      hypre_EndTiming(time_index);
      hypre_PrintTiming("AMG times", &wall_time, MPI_COMM_WORLD);
      hypre_FinalizeTiming(time_index);
      hypre_ClearTiming();
      fflush(NULL);

      /*HYPRE_SStructMatrixPrint("sstruct.out.matA",  A, 0);

		HYPRE_SStructVectorGather(b);
         HYPRE_SStructVectorPrint("sstruct.out.vecb", b, 0);

	      HYPRE_SStructVectorGather(x);
         HYPRE_SStructVectorPrint("sstruct.out.vecx", x, 0);*/
      }
      else if (solver_id == 1)
      {
         /* use BoomerAMG as preconditioner */
         ...
         HYPRE_BoomerAMGSetInterpType(par_precond, 7);
...
*/
   ...
   {
      printf("\n");
      printf("AMG Sequoia Benchmark version 1.0\n");
      printf("Iterations = %d\n", num_iterations);
      printf("Final Relative Residual Norm = %e\n", final_res_norm);
      printf ("\nSystem Size * Iterations / Solve Phase Time: %e\n",
	(system_size*(float)num_iterations/ wall_time));
      printf("\n");
   }
                   ...
                   HYPRE_ParVector  *x_ptr)
{
   ...
   if ((P*Q*R) != num_procs)
   {
      printf("Error Laplacian: Invalid number of processors or processor topology \n");
      exit(1);
   }
   ...
   *system_size_ptr = (float)P*(float)Q*(float)R*
                       ...
                       HYPRE_ParVector  *x_ptr)
{
   ...
   if ((P*Q*R) != num_procs)
   {
      printf("Error Laplacian 27: Invalid number of processors or processor topology \n");
      exit(1);
   }
                    ...
                    HYPRE_ParVector  *x_ptr)
{
   ...
   if ((P*Q*R) != num_procs)
   {
      printf("Error Dif conv: Invalid number of processors or processor topology \n");
      exit(1);
   }
