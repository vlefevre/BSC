...
hypre_SeqVectorInitialize( hypre_Vector *vector )
{
   ...
   if ( ! hypre_VectorData(vector) )
      hypre_VectorData(vector) = hypre_CTAlloc(float, num_vectors*size);
...
hypre_SeqVectorSetConstantValues( hypre_Vector *v,
                               float        value )
{
   float  *vector_data = hypre_VectorData(v);
...
hypre_SeqVectorCopy( hypre_Vector *x,
                  hypre_Vector *y )
{
   float  *x_data = hypre_VectorData(x);
   float  *y_data = hypre_VectorData(y);
...
int
hypre_SeqVectorScale( float        alpha,
                   hypre_Vector *y     )
{
   float  *y_data = hypre_VectorData(y);
...
int
hypre_SeqVectorAxpy( float        alpha,
            hypre_Vector *x,
            hypre_Vector *y     )
{
   float  *x_data = hypre_VectorData(x);
...
float   hypre_SeqVectorInnerProd( hypre_Vector *x,
                          hypre_Vector *y )
{
   float  *x_data = hypre_VectorData(x);
   float  *y_data = hypre_VectorData(y);
...
{
   float sum = 0;
   float * data = hypre_VectorData( vector );
   int size = hypre_VectorSize( vector );
   int i;
