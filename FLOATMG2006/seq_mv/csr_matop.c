              ...
              hypre_CSRMatrix *B)
{
   ...
   float     *B_data   = hypre_CSRMatrixData(B);
...
hypre_CSRMatrix *
hypre_CSRMatrixDeleteZeros( hypre_CSRMatrix *A, float tol)
{
   float     *A_data   = hypre_CSRMatrixData(A);
   int        *A_i      = hypre_CSRMatrixI(A);
   int        *A_j      = hypre_CSRMatrixJ(A);
...

{
   ...
   *AT = hypre_CSRMatrixCreate(num_rowsAT, num_colsAT, num_nonzerosAT);
   ...
   if (data) 
   {
      AT_data = hypre_CTAlloc(float, num_nonzerosAT);
...
{
   float sum = 0;
   float * data = hypre_CSRMatrixData( A );
   int num_nonzeros = hypre_CSRMatrixNumNonzeros(A);
   int i;
