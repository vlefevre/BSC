...
/*   int  num_rownnz = hypre_CSRMatrixNumRownnz(matrix); */
   ...
   if ( ! hypre_CSRMatrixData(matrix) && num_nonzeros )
      hypre_CSRMatrixData(matrix) = hypre_CTAlloc(float, num_nonzeros);
   ...
   HYPRE_BigInt ** col_map_offd_C )
{
   ...
   if ( col_map_offd_A )
   {
      *col_map_offd_C = hypre_CTAlloc( HYPRE_BigInt, num_cols );
   ...
   /* ==== The first run through A and B is to count the number of nonzero elements,
      without float-counting duplicates.  Then we can create C. */
