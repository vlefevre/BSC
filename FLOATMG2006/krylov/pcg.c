...
hypre_PCGFunctionsCreate(
   ...
   int    (*Matvec)        ( void *matvec_data, float alpha, void *A,
                             void *x, float beta, void *y ),
   int    (*MatvecDestroy) ( void *matvec_data ),
   float (*InnerProd)     ( void *x, void *y ),
   int    (*CopyVector)    ( void *x, void *y ),
   int    (*ClearVector)   ( void *x ),
   int    (*ScaleVector)   ( float alpha, void *x ),
   int    (*Axpy)          ( float alpha, void *x, void *y ),
                ...
                void *x         )
{
   ...
   if ( (pcg_data->logging)>0  || (pcg_data->print_level)>0 ) 
   {
      if ( (pcg_data -> norms) != NULL )
         hypre_TFreeF( pcg_data -> norms, pcg_functions );
      (pcg_data -> norms)     = hypre_CTAllocF( float, max_iter + 1,
                                                pcg_functions);

      if ( (pcg_data -> rel_norms) != NULL )
         hypre_TFreeF( pcg_data -> rel_norms, pcg_functions );
      (pcg_data -> rel_norms) = hypre_CTAllocF( float, max_iter + 1,
                                                pcg_functions );
   }
                ...
                void *x         )
{
   ...
   float          tol          = (pcg_data -> tol);
   float          atolf        = (pcg_data -> atolf);
   float          cf_tol       = (pcg_data -> cf_tol);
   int             max_iter     = (pcg_data -> max_iter);
...
*/
   ...
   float         *norms        = (pcg_data -> norms);
   float         *rel_norms    = (pcg_data -> rel_norms);
                
   float          alpha, beta;
   float          gamma, gamma_old;
   float          bi_prod, i_prod, eps;
   float          pi_prod, xi_prod;
   float          ieee_check = 0.;
   ...
   float          guard_zero_residual, sdotp; 
   ...
   precond(precond_data, A, r, p); //compute an approximate solution to Ar = p (p = A^(-1)*r, instead of computing an approximate A^-1 and then do the matrix multiplication*/
   ...
   while ((i+1) <= max_iter)
   {
      ...
      /* alpha = gamma / <s,p> */
      sdotp = (*(pcg_functions->InnerProd))(s, p); /*sdotp = dT A d*/
      if ( sdotp==0.0 )
      {
         ++ierr;
                                       ...
                                       float *relative_residual_norm )
{
   ...
   float         rel_residual_norm = (pcg_data -> rel_residual_norm);
