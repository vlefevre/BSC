                  ...
                  void *x         )
{
   ...
   {
      if ((gmres_data -> norms) == NULL)
         (gmres_data -> norms) = hypre_CTAllocF(float, max_iter + 1,gmres_functions);
   }
		 ...
		 void  *x)
{
   ...
   float 	     accuracy     = (gmres_data -> tol);
   float 	     cf_tol       = (gmres_data -> cf_tol);
...
/*   FILE           *fp; */
   ...
   int        iter; 
   int        my_id, num_procs;
   float     epsilon, gamma, t, r_norm, b_norm, den_norm, x_norm;
   float     epsmac = 1.e-16; 
   float     ieee_check = 0.;

   float     guard_zero_residual; 
   float     cf_ave_0 = 0.0;
   float     cf_ave_1 = 0.0;
   float     weight;
   float     r_norm_0;
   ...
   rs = hypre_CTAllocF(float,k_dim+1,gmres_functions); 
   c = hypre_CTAllocF(float,k_dim,gmres_functions); 
   s = hypre_CTAllocF(float,k_dim,gmres_functions); 

   hh = hypre_CTAllocF(float*,k_dim+1,gmres_functions); 
   for (i=0; i < k_dim+1; i++)
   {	
   	hh[i] = hypre_CTAllocF(float,k_dim,gmres_functions); 
   }
