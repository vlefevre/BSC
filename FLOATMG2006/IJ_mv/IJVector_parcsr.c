...
/* Determine whether indices points to local indices only,
   ...
   if (indices)
   {
      ...
      float *off_proc_data = hypre_AuxParVectorOffProcData(aux_vector);
   			     	      ...
   			     	      float *off_proc_data)
{
   ...
   float *send_data;
   float *recv_data;
   float *data = hypre_VectorData(hypre_ParVectorLocalVector(par_vector));
   ...
   send_data = hypre_CTAlloc(float,send_map_starts[num_sends]);
   recv_i = hypre_CTAlloc(HYPRE_BigInt,recv_vec_starts[num_recvs]);
   recv_data = hypre_CTAlloc(float,recv_vec_starts[num_recvs]);
   			     	      ...
   			     	      float *off_proc_data)
{
  ...
  /*create response object - can use same fill response as used in the commpkg
   ...
   /* now we have the list of real procesors ids (real_proc_id) - 
      ...
      so now we can set up data to be sent - we have int and float data.
      (row number and value) - we will send everything as a void since
       we may not know the rel sizes of ints and floats*/  
  ...
  /***********************************/
   ...
   for (i=0; i < num_recvs; i++)
   {
      ...
      for (j=0; j < row_count; j++) /* for each row: unpack info */
      {
         ...
         /* value */
         memcpy( &value, recv_data_ptr, float_size);
         recv_data_ptr = (void *) ((char *)recv_data_ptr + obj_size_bytes);
         indx++;
