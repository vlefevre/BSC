         ...
         else
	 {
	    ...
	    if (!max_off_proc_elmts)
	    {
	       ...
	       hypre_AuxParCSRMatrixMaxOffProcElmts(aux_matrix) =
			...
			= hypre_CTAlloc(float,max_off_proc_elmts);
   	       off_proc_i = hypre_AuxParCSRMatrixOffProcI(aux_matrix);
   	       off_proc_j = hypre_AuxParCSRMatrixOffProcJ(aux_matrix);
   	       off_proc_data = hypre_AuxParCSRMatrixOffProcData(aux_matrix);
	    }
            else if (current_num_elmts + n > max_off_proc_elmts)
            {
               ...
               off_proc_data = hypre_TReAlloc(off_proc_data,float,
				max_off_proc_elmts);
	       hypre_AuxParCSRMatrixMaxOffProcElmts(aux_matrix)
   	    		= max_off_proc_elmts;
   	       ...
   	       if (size < n)
      	       {
      	          ...
      	          tmp_data = hypre_CTAlloc(float,size);
      	       }
      	       else
      	       {
      	          tmp_j = NULL;
...
hypre_IJMatrixAssembleParCSR(hypre_IJMatrix *matrix)
{
   ...
   /* move data into ParCSRMatrix if not there already */ 
      if (hypre_AuxParCSRMatrixNeedAux(aux_matrix))
      {
         ...
         diag_data = hypre_CTAlloc(float,i_diag);
         if (i_offd > 0)
         {
    	    offd_j = hypre_CTAlloc(int,i_offd);
            offd_data = hypre_CTAlloc(float,i_offd);
   					 ...
   					 float *off_proc_data)
{
   ...
   send_data = hypre_CTAlloc(float,dbl_send_map_starts[num_sends]);
   recv_i = hypre_CTAlloc(HYPRE_BigInt,recv_vec_starts[num_recvs]);
   recv_data = hypre_CTAlloc(float,dbl_recv_vec_starts[num_recvs]);
   ...
   for (i=0; i < num_recvs; i++)
   {
       ...
       ip = recv_procs[i];
       MPI_Irecv(&recv_data[vec_start], vec_len, MPI_FLOAT, ip, 0, comm, 
			&requests[j++]);
   }
   ...
   for (i=0; i < num_sends; i++)
   {
       ...
       ip = send_procs[i];
       MPI_Isend(&send_data[vec_start], vec_len, MPI_FLOAT, ip, 0, comm, 
			&requests[j++]);
   }
   					 ...
   					 float *off_proc_data)
{
   ...
   int  obj_size_bytes, int_size, float_size, big_int_size;
   int  tmp_int;
   HYPRE_BigInt tmp_big_int;
   HYPRE_BigInt *col_ptr;
   HYPRE_BigInt *big_int_data = NULL;
   int big_int_data_size = 0, float_data_size = 0;
 ...
 /* loop through ranges - create a list of actual processor ids*/
   ...
   /* now we have the list of real procesors ids (real_proc_id) - and
      ...
      sent - we have int data and float data.  that we will need to
      pack together */
   ...
   /* to pack together, we need to use the largest obj. size of
      (int) and (float) - if these are much different, then we are
   ...
   obj_size_bytes = hypre_max(big_int_size, float_size);
   ...
   for (i=0; i < num_rows; i++)
   {
      ...
      for (j=0; j< num_elements; j++)
      {
         tmp_float = off_proc_data[counter++]; /* value */

         memcpy( index_ptr, &tmp_float, float_size);
         index_ptr = (void *) ((char *) index_ptr + obj_size_bytes);
         in_i++;
   ...
   for (i=0; i < num_recvs; i++)
   {
      ...
      for (j=0; j < num_rows; j++) /* for each row: unpack info */
      {
         ...
         /* col data */
         if (float_size == obj_size_bytes)
         {
            col_data_ptr = (float *) recv_data_ptr;
            recv_data_ptr = (void *) ((char *)recv_data_ptr + num_elements*obj_size_bytes);
         }
         else /* copy data */
         {
            if (float_data_size < num_elements)
            {
               float_data = hypre_TReAlloc(float_data, float, num_elements + 10);
            }
            for (k=0; k< num_elements; k++)
            { 
               memcpy( &float_data[k], recv_data_ptr, float_size);
               recv_data_ptr = (void *) ((char *)recv_data_ptr + obj_size_bytes);
            }
            col_data_ptr = float_data;
   ...
   if (float_data) hypre_TFree(float_data);
...

{
   ...
   object_size = hypre_max(sizeof(HYPRE_BigInt), sizeof(float));
