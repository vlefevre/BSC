...
%     \includegraphics[width=\textwidth]{figs/3157_15.pdf}
   ...
   Figures~\ref{fig.estimation1} to~\ref{fig.estimation5} represents the cost of the algorithm, for these different scenarios, to reach different accuracy in the output ($10^{-3}$, $10^{-7}$ and $10^{-15}$ here) using
   ...
   accuracy $10^{-3}$ (see Figure~\ref{fig.estimation1}). If one adapts the algorithm to use only single-precision variables (labelled \texttt{24\_0}) as it would be sufficient to reach this accuracy, we would still reduce the time by 17\% (still Figure~\ref{fig.estimation1}). Then the improvement slowly decreases when increasing the accuracy to reach 10\% (on Figure~\ref{fig.estimation5}).
