...
\def\pch{\mathcal{P}^{c}\xspace}
\def\pcomp{\mathcal{P}^{a}\xspace}
\def\tcomp{T_{a}\xspace}
\def\pres{\mathcal{P}^{r}\xspace}
\def\pidle{\mathcal{P}^{idle}\xspace}
\def\Wch{\mathfrak{W}^{ch}}
\def\Wrec{\mathfrak{W}^{rew}}
\def\Wdown{\mathfrak{W}^{down}}
\def\Ech{\mathcal{E}^{ch}}
\def\Erec{\mathcal{E}^{rew}}
\def\Edown{\mathcal{E}^{down}}
\def\Wtime{\mathbb{W}}
\def\Wenergy{\mathbb{E}}
\def\Topt{\tau_{time}^{*}}
\def\Eopt{\tau_{energy}^{*}}
\def\Pratio{\rho}
...
\usebackgroundtemplate{\includegraphics[width=\paperwidth,height=\paperheight]{figures/bg.jpeg}}

% Set Color ==============================
% Custom colors
\usepackage{xcolor}

\definecolor{clea1}{HTML}{FFFFFF}
\definecolor{clea2}{HTML}{CCCCCC}
\definecolor{clea3}{HTML}{999999}
\definecolor{trix1}{HTML}{CCFFFF}
\definecolor{trix2}{HTML}{CCFFCC}
\definecolor{trix3}{HTML}{FFCCCC}
\definecolor{dark1}{HTML}{666666}
\definecolor{dark2}{HTML}{333333}
\definecolor{dark3}{HTML}{000000}

\setbeamercolor{block title}            {fg=dark1, bg=clea3}
\setbeamercolor{block body}             {fg=dark3, bg=clea1}
\setbeamercolor{block title alerted}    {fg=clea3, bg=dark3}
\setbeamercolor{block body alerted}     {fg=clea2, bg=dark2}
\setbeamercolor{block title example}    {fg=dark2, bg=trix3}
\setbeamercolor{block body example}     {fg=dark3, bg=clea3}

\setbeamercolor{normal text}            {fg=clea1, bg=dark1}
\setbeamercolor{background canvas}      {fg=clea2, bg=dark2}
\setbeamercolor{background}             {fg=clea3, bg=dark3}

\setbeamercolor{palette primary}        {fg=clea2, bg=dark2}
\setbeamercolor{palette secondary}      {fg=clea1, bg=dark1}
\setbeamercolor{palette tertiary}       {fg=clea3, bg=dark3}

\setbeamercolor{frametitle}             {fg=dark1, bg=trix1}
\setbeamercolor{title}                  {fg=dark1, bg=trix1}

\setbeamertemplate{title page}[default][colsep=-8bp,rounded=true]
...
\newenvironment{myblock}{%
    ...
    \tikz\node[
        fill=clea1,
        draw=dark2,
        line width=0.5pt, style=dashed,
        inner sep=10pt,
        outer sep=0pt,
    ]{\usebox\blockbox};
}

\newcommand{\tx}[1]{\large\textcolor{dark3}{#1}}
\newcommand{\ts}[1]{\scriptsize\textcolor{dark3}{#1}}


%\else
%\fi

%\title{9th PMBS Workshop @ SC18}
...
\end{figure}
 \vspace{-0.5cm}
 GPU compared to double-precision: \textbf{\only<1>{34}\only<2>{23}\only<3>{9}\%} improvement.
