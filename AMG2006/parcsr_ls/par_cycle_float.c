   ...
   while (Not_Finished)
   {
	...
	t1 = clock();
      ...
      for (jj = 0; jj < cg_num_sweep; jj++)
      {
         ...
         for (j = 0; j < num_sweep /*|| (j<num_sweep*2 && (num_levels-level-3)%2==0 && level != 0 && level != 1 && level!=num_levels-1 )*/; j++)
         {
		...
		mpfr_t omega_f, relax_weight_f;
		mpfr_init2(omega_f,PRECISION_MPFR); mpfr_init2(relax_weight_f,PRECISION_MPFR);
		mpfr_set_d(omega_f,omega[level],MPFR_RNDN);
		mpfr_set_d(relax_weight_f,relax_weight[level],MPFR_RNDN);
	    if (old_version)
	    {
