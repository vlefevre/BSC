...
ax1.semilogy(V_1_fast2[:,0],V_1_fast2[:,1],color='b',label='V_1_fast2')
...
print slope,intercept
ax2.scatter(V_1_fast2[:,0],V_1_fast2[:,2],color='b',marker='.',s=10,alpha=0.5)
slope, intercept, r_value, p_value, std_err = stats.linregress(V_1_fast2[:,0],V_1_fast2[:,2])
ax2.plot(V_1_fast2[:,0],slope*V_1_fast2[:,0]+intercept,color='b',linestyle='--')
print slope,intercept
...
fig.tight_layout()
pylab.savefig('convergence_fast_small.pdf')
pylab.clf()
